...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const sorted = [a, b].sort((i, j) => i - j);
  const start = isStartIncluded ? '[' : '(';
  const end = isEndIncluded ? ']' : ')';

  return `${start}${sorted.join(', ')}${end}`;
}
...
function getDigitalRoot(num) {
  const sum = [...String(num)].reduce((a, b) => Number(a) + Number(b));
  const str = String(sum);

  if (str.length > 1) return Number(str[0]) + Number(str[1]);
  return Number(str);
}
